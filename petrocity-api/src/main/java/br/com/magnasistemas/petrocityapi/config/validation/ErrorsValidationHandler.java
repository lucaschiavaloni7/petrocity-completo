...
public class ErrorsValidationHandler {
	
	@Autowired
	private MessageSource messageSource;

	@ResponseStatus(code = HttpStatus.BAD_REQUEST)
	//MethodArgumentNotValidException é a exceção gerada na validação do formulário
	...
	public List<ErrorFormDto> handle(MethodArgumentNotValidException exception){
		...
		for (FieldError fieldError : fields) {
			...
			errors.add(new ErrorFormDto(fieldError.getField(), message));
		}
